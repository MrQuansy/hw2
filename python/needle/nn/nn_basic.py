...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        var = ((x_minus_mean ** 2).sum(axes=(1, )).reshape((batch_size, 1)) / feature_size + self.eps) ** 0.5
        norm = x_minus_mean / var.broadcast_to(x.shape)
        return self.weights.broadcast_to(x.shape) * norm + self.bias.broadcast_to(x.shape)
        ### END YOUR SOLUTION
